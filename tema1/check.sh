...
if [[ $1 =~ $numberPattern ]]; then
    ...
    if [ $# -gt 1 ] && [ $2 -eq 1 ]; then
        diff -b server.out tests/test$1/expected_output/server.out
        if [[ $? = 1 ]]; then
            echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ SERVER OUTPUT DIFF"
            echo
        fi

        diff -b client.out tests/test$1/expected_output/client.out
        if [[ $? = 1 ]]; then
            echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ CLIENT OUTPUT DIFF"
            echo
        fi
    else
        diff -b server.out tests/test$1/expected_output/server.out > /dev/null
        diff -b client.out tests/test$1/expected_output/client.out > /dev/null
    fi
    ...
    do
        ...
        diff -b server.out tests/test$i/expected_output/server.out > /dev/null
        if [[ $? = 0 ]]; then
        ...
        diff -b client.out tests/test$i/expected_output/client.out > /dev/null
        if [[ $? = 0 ]]; then
